...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Valida configuración, dataset y simulador sin llamar a Ollama.")
    parser.parse_args(argv)
    try:
        config = load_config()
        cases_doc = json.loads((ROOT / "datasets" / "benchmark_cases_v1.json").read_text(encoding="utf-8"))
        fixtures = json.loads((ROOT / "datasets" / "fixtures_v1.json").read_text(encoding="utf-8"))
        tools_doc = json.loads((ROOT / "datasets" / "tools_v1.json").read_text(encoding="utf-8"))
